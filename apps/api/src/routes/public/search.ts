...
function createSearchRouter(database: typeof db = db) {
  const app = new Hono()
  ...
  app.get('/', zValidator('query', searchQuerySchema), async (c) => {
    try {
      ...
      const suggestions = total < 5 && page === 1
        ? await generateSearchSuggestionsInternal(q, category, domain)
        : undefined
  ...
  app.get('/suggestions', zValidator('query', suggestionsQuerySchema), async (c) => {
    try {
      ...
      if (!type || type === 'title') {
        // 使用 FTS5 从标题中查找建议
        const titleSuggestions = await database
      ...
      if (!type || type === 'category') {
        // 使用 FTS5 从分类中查找建议
        const categorySuggestions = await database
          .select({ 
            category: links.userCategory,
            count: count()
          })
      ...
      if (!type || type === 'domain') {
        // 使用 FTS5 从域名中查找建议
        const domainSuggestions = await database
          .select({
            domain: links.domain,
            count: count()
          })
  ...
  async function generateSearchSuggestionsInternal(
    query: string, 
    category?: string, 
    domain?: string
  ): Promise<string[]> {
